...
load('X_train.txt')
load('y_train.txt')
 labels=y_train(1:500,1)
 l=labels
            X=X_train(1:500,1:561)
...
nbytes = fprintf(fileID,'%5d %5d %5d %5d %5d %5d %5d %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n',population_init);
 end
fclose(fileID);
File = csvread('K_M_mobile.csv');

[population front]=NDS_CD_cons(File);

csvwrite('K_M_S_mobile.csv',population)