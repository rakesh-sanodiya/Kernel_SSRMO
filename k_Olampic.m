...
fileID = fopen('K_M_olam.csv','w');
 V=17
 M=3
 for r = 1:1
%% load dataset
load('olympic_circles.mat')
gt_data=transpose(gt_data) 
 labels=gt_data(1:500,1)
 l=labels
            X=X(1:500,1:2)
...
% use k = no_clusters
idx_multi = perfkkmeans(K,5,200); % repeat kernel k-means 20 times
...
nbytes = fprintf(fileID,'%5d %5d %5d %5d %5d %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n',population_init);
 end
fclose(fileID);
File = csvread('K_M_olam.csv');

[population front]=NDS_CD_cons(File);

csvwrite('K_M_S_olam.csv',population)