...
fileID = fopen('K_M_ring2.csv','w');
 V=14
 M=3
 for r = 1:1
%% load dataset
load('2ring.txt')
 labels=X2ring(1:300,3)
 l=labels
            X=X2ring(1:300,1:2)
...
% use k = no_clusters
idx_multi = perfkkmeans(K,2,200); % repeat kernel k-means 20 times
...
fclose(fileID);
File = csvread('K_M_ring2.csv');

[population front]=NDS_CD_cons(File);

csvwrite('K_M_S_2ring.csv',population)
for r = 1:30
  fileID = fopen('Multi_RING.csv','w'); 
    l=population(1:4,4)
    ch1=int2str(l(1,1))
    ch2=int2str(l(2,1))
    ch3=int2str(l(3,1))
...
nbytes = fprintf(fileID,'%5d %5d %5d %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n',population_init);
            ...
            fclose(fileID);
File = csvread('K_M_S_RING.csv');

[population front]=NDS_CD_cons(File);

csvwrite('Multi_M_S_RING.csv',population)
